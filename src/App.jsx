...
function Card({ children, style }) {
  const T = useT();
  return <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: "20px 22px", ...style }}>{children}</div>;
}
...
function useInputStyle() {
  const T = useT();
  return { width: "100%", padding: "10px 12px", border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 13.5, background: T.inputBg, color: T.text, outline: "none" };
}
...
export default function FinanceTracker() {
  ...
  useEffect(() => {
    ...
    saveData({ accounts, transactions, goalsList, budgets, theme });
